...
class RemoteConfig:
    ...
    server_port: int = 6160
    listener_port: int = 10002  # 本地监听端口，用于接收任务切换命令

    task_descriptions_json: str = (
        '{"pick_bowls": "use the left arm to place the cup into the bowl, then use the right arm to pick up the bowl", '
        '"put_bowls": "use the right arm to put the bowl in the sink", '
        '"pick_mouthwash_table": "use the right arm to pick up the bottle from table", '
        '"pick_mouthwash_cabinet": "use the right arm to pick up the bottle from cabinet", '
        '"pick_cup": "use the left arm to pick up the cup from table, grasping the left side of the cup", '
        '"put_cup": "use the left arm to put the cup in the trash can by the table"}'
    )
    stop_thresh: float = 0.001  # 统一停止阈值
    stop_stable_frames: int = 100  # 统一稳定帧数
    max_steps: int = 1000  # 统一的最大步数
    ...
    def __post_init__(self):
        ...
        if isinstance(self.task_descriptions_json, str):
            self.task_descriptions = json.loads(self.task_descriptions_json)
        else:
            self.task_descriptions = self.task_descriptions_json
...
class AIRBOT_Controller:
    ...
    def __init__(
        ...
        self.server_url = f"http://{self.target_ip}:{self.server_port}"

        self.task_descriptions = task_descriptions

        self.stop_thresh = stop_thresh
        self.stop_stable_frames = stop_stable_frames
        ...
        self.url_lock = threading.Lock()  # 保护 _current_task
        self.buffer_lock = threading.Lock()  # 保护 action_buffer

        cprint(f"[*] AIRBOT PLAY 控制器初始化完成。", "green")
        cprint(f"[*] 统一服务器目标: {self.server_url}", "green")
        cprint(
            f"[*] 全局停止阈值: THRESH={self.stop_thresh}, STABLE_FRAMES={self.stop_stable_frames}",
            "green",
        )
        cprint(f"[*] 全局最大步数: MAX_STEPS={self.max_steps}", "green")
    ...
    # --- 3. 机器人控制与通信 (消费者) ---
    # ----------------------------------------------------------------
    def _clear_remote_cache(self, task_name: str):
        """向推理服务器发送 clear_cache 请求, 可选地附带任务描述"""
        endpoint = self.server_url + "/clear_cache"
        task_description = self.task_descriptions.get(task_name)

        payload = {}
        if task_description:
            payload = {"task_description": task_description}
            self.safe_cprint(f"[*] 发现任务描述: {task_description}", "cyan")
        else:
            self.safe_cprint(
                f"[*] 未找到任务 '{task_name}' 的特定描述, 仅清除缓存。", "yellow"
            )

        try:
            resp = self.session.post(
                endpoint,
                json=payload,  # 发送 JSON
            ...
            if resp.status_code == 200:
                ...
                return False, f"Server returned error: {resp.status_code}, {resp.text}"
    ...
    def start_task(self, task_name: str):
        ...
        self.safe_cprint(f"[*] 正在测试连接到: {self.server_url}", "cyan")
        if not self.test_connection():
            self.safe_cprint("[!] 服务器连接失败，退出控制循环。", "red")
            return False, "Server connection test failed"

        # --- NEW: 调用 clear_cache ---
        self.safe_cprint(f"[*] 正在清除远程缓存并设置任务: {task_name}", "cyan")
        success, clear_msg = self._clear_remote_cache(task_name)
        if not success:
            self.safe_cprint(f"[!] 远程缓存清除失败: {clear_msg}", "red")
            with self.state_lock:
                self.is_running = False  # 确保状态回滚
            return False, f"Failed to clear remote cache: {clear_msg}"
        self.safe_cprint(f"[✔] 远程缓存已清除。", "green")
        # --- END NEW ---
    ...
    def _check_task_complete(self, qpos):
        ...
        # 阈值设置（单位：弧度/帧）
        THRESH = self.stop_thresh
        STABLE_FRAMES = self.stop_stable_frames
    ...
    def run_control_loop(self):
        ...
        try:
            while step < self.max_steps and not self.control_loop_stop_event.is_set():
                ...
                if error or action is None:
                    ...
                    status_msg = (
                        f"🟢 控制状态: 运行中... (第 {step} / {self.max_steps} 步)"
                    )
                ...
                config_str = f"   - 停止配置: Thresh={self.stop_thresh} | Frames={self.stop_stable_frames}"
...
def main():
    ...
    try:
        controller = AIRBOT_Controller(
            ...
            task_descriptions=opt.task_descriptions,
            stop_thresh=opt.stop_thresh,
            stop_stable_frames=opt.stop_stable_frames,
